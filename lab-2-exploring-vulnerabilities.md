# Lab 2: Exploring vulnerabilities
...
ZAP is used on GitLab as the tool to perform Dynamic Application Security Testing which we will explore later.

Make sure that Juice Shop is still running from the previous lab :

```bash
sudo docker run -p 3000:3000 bkimminich/juice-shop
```

Open Firefox and navigate to the URL if not already http://127.0.0.1:3000/

We can now run ZAP by selecting it from the menu (Applications > Pentesting > Web Application Analysis > owasp-zap):

<figure><img src=".gitbook/assets/Screenshot 2024-06-28 at 11.26.33.png" alt=""><figcaption></figcaption></figure>

Open ZAP and configure the software to scan the Juice Shop website:

1. Decide if you want to persist the session. You can say yes with current timestamp.
2. You may be prompted to update all of the scanner rules. If so, select Update All. You can then close that window.
3. Select Automated Scan from the Welcome to Zap screen.
4. Type in the URL of the Juice Shop `http://127.0.0.1:3000`&#x20;
5. Tick both `User traditional spider` and `Use ajax spider with Firefox Headless`, then click "Attack".
...
### 4. First Erorr: Read the FTP files
...
Why is this a vulnerability? Well, for a start it didn't require a username and password to access, in other words, it allowed _anonymous_ access. Secondly, as mentioned previously, sensitive files should not be left on servers without encryption of any sort and even then, they should be made available only to the people who need to see it. So this is one of OWASP's _Broken Access Control_.

### 5. Second Error: Error handling
...
To find this error, go back to ZAP and click on the `Active Scan` tab in the bottom window. Sort the output by Code with the sort order showing the largest codes at the top. look until you can find a code of 500 which is an Internal Server Error. One of them is caused by an unexpected path to `api`. You should open the link in your browser `http://127.0.0.1:3000/api`. You should see something like this:

<figure><img src=".gitbook/assets/Screenshot 2024-06-28 at 13.31.04.png" alt=""><figcaption></figcaption></figure>
...
Click on the `Storage` tab (e.g., in Chrome, this is under `Application` tab) and then find under `Cookies` the cookie for `http://127.0.0.1:3000` and click that. You should see one of the cookies is `token` - copy the contents of that and paste it after `Authorization: Bearer` in the request header. Now press `Send`.
